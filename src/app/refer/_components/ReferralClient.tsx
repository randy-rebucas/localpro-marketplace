...
export default function ReferralClient({ referralCode, referralLink }: ReferralClientProps) {
  ...
  const shareText = `Join me on LocalPro — the trusted marketplace for local service professionals in the Philippines! Use my link to sign up: ${referralLink}`;
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-6 space-y-5">
      <h2 className="font-bold text-slate-900 text-lg">Your Referral Link</h2>
      ...
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 text-sm px-3 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary/30 font-mono"
        />
        <button
          onClick={copyLink}
      ...
      <div className="flex items-center gap-3">
        <div className="flex-1">
          <p className="text-xs text-slate-500 mb-1">Or share your code</p>
          <div className="flex gap-2 items-center">
            <code className="text-lg font-mono font-bold text-primary tracking-widest bg-primary/5 px-4 py-2 rounded-xl">
              {referralCode}
            </code>
            <button
              onClick={copyCode}
      ...
      <div>
        <div className="flex items-center gap-1.5 mb-3">
          ...
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Share on</p>
        </div>
        <div className="flex gap-2 flex-wrap">
          {shareChannels.map((ch) => (
            <a
              ...
              className={`flex items-center justify-center w-10 h-10 rounded-xl ${ch.bg} hover:opacity-80 transition-opacity`}
            >
              {ch.icon}
            </a>
          ))}
