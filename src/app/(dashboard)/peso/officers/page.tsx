...
import { Users, UserPlus, Trash2, Building2, ShieldCheck } from "lucide-react";
...
export default function OfficersPage() {
  ...
  async function handleAddOfficer(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/peso/officers", {
        method: "POST",
  ...
  async function handleRemove(officerUserId: string, officerName: string) {
    ...
    try {
      const res = await fetch(`/api/peso/officers/${officerUserId}`, { method: "DELETE" });
      const data = await res.json();
    ...
    <div className="max-w-2xl mx-auto space-y-5">
      ...
      <div className="bg-blue-700 rounded-xl px-6 py-5 text-white">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest opacity-70">PESO Office</p>
            <p className="text-lg font-bold mt-0.5">{office.officeName}</p>
            <p className="text-sm opacity-80 mt-0.5">{office.municipality}, {office.region}</p>
            <p className="text-xs opacity-60 mt-1">{office.contactEmail}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        {/* Add officer form */}
        {showForm && isHead && (
          <form onSubmit={handleAddOfficer} className="px-5 py-4 border-b border-slate-100 bg-blue-50 space-y-3">
            <p className="text-xs font-semibold text-blue-600 uppercase tracking-wide">Add New Officer</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  className="mt-1 w-full border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-700 outline-none focus:ring-1 focus:ring-blue-400 bg-white"
                />
              </div>
              <div>
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-slate-50">
          <div className="flex items-center gap-3">
            <div className="h-8 w-8 rounded-full bg-blue-700 flex items-center justify-center text-white text-xs font-bold shrink-0">
              {office.headOfficerId.name?.slice(0, 2).toUpperCase() ?? "HO"}
            </div>
            <div>
              <p className="text-sm font-medium text-slate-800">{office.headOfficerId.name}</p>
              <p className="text-xs text-slate-400">{office.headOfficerId.email}</p>
            </div>
          </div>
        ...
        {office.officerIds?.length === 0 ? (
          <div className="px-5 py-6 text-center text-slate-400 text-sm">
            No staff officers yet.{isHead && " Click \"Add Officer\" to invite one."}
          </div>
        ) : (
              ...
              <div className="flex items-center gap-3">
                ...
                {isHead && (
                  <button
                    ...
                    disabled={removingId === officer._id}
