...
import { Headphones, User, Ticket } from "lucide-react";
...
export default function AdminSupportShell({ threads, children }: Props) {
  ...
  const isTickets = pathname.startsWith("/admin/support/tickets");

  const totalUnread = threads.reduce((s, t) => s + t.unreadForAdmin, 0);

  return (
    <div className="flex flex-col h-[calc(100vh-8rem)] overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
      {/* ── Top: tab strip ────────────────────────────────────────────── */}
      <div className="flex items-center gap-4 px-4 py-3 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/80 shrink-0">
        <div className="flex items-center gap-2">
          <div className="p-1.5 rounded-lg bg-primary/10">
            <Headphones className="h-3.5 w-3.5 text-primary" />
          </div>
          <h2 className="text-sm font-semibold text-slate-800 dark:text-white">Support</h2>
          {totalUnread > 0 && (
        ...
        <div className="flex gap-1 rounded-lg bg-slate-200 dark:bg-slate-700 p-0.5">
          <Link
            href="/admin/support"
            className={`flex items-center gap-1.5 px-3 py-1 rounded-[5px] text-xs font-medium transition-colors ${
              !isTickets
                ? "bg-white dark:bg-slate-600 text-slate-800 dark:text-white shadow-sm"
                : "text-slate-500 dark:text-slate-400 hover:text-slate-700"
            }`}
          >
            <Headphones className="h-3 w-3" /> Live Chat
          </Link>
          <Link
          ...
          >
            <Ticket className="h-3 w-3" /> Ticket Queue
          </Link>
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Left: thread list (hidden on Tickets view) */}
        {!isTickets && (
