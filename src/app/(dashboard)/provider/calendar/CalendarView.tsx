...
// Mobile: single-letter  Desktop: 3-letter
const DAY_SHORT  = ["S", "M", "T", "W", "T", "F", "S"];
const DAY_MEDIUM = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
const MONTH_NAMES = [
...
function JobChip({ job, today, onClick }: { job: CalendarJob; today: Date; onClick: () => void }) {
  ...
  return (
    <button
      ...
      title={job.title}
      className={`w-full text-left truncate text-[10px] font-medium px-1 sm:px-1.5 py-0.5 rounded border ${
...
// ─── JobDot — compact indicator for mobile month cells ───────────────────────

function JobDots({ jobs, today }: { jobs: CalendarJob[]; today: Date }) {
  const MAX = 3;
  return (
    <div className="flex items-center gap-0.5 mt-0.5 justify-center">
      {jobs.slice(0, MAX).map((j) => {
        const overdue = isOverdue(j, today);
        const c = STATUS_COLOR[j.status] ?? { dot: "bg-slate-400" };
        return (
          <span
            key={j._id}
            className={`h-1.5 w-1.5 rounded-full flex-shrink-0 ${overdue ? "bg-red-500" : c.dot}`}
          />
        );
      })}
      {jobs.length > MAX && (
        <span className="text-[9px] text-slate-400 leading-none">+{jobs.length - MAX}</span>
      )}
    </div>
  );
}
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-3">
        <div className="bg-white rounded-xl border border-slate-200 shadow-card px-2 py-2 sm:px-4 sm:py-3 flex items-center gap-1.5 sm:gap-3">
          <span className="hidden sm:flex h-9 w-9 items-center justify-center rounded-lg bg-primary/10 text-primary flex-shrink-0">
            <Briefcase className="h-4 w-4" />
          </span>
          <div>
            <p className="text-[10px] sm:text-[11px] text-slate-500 font-medium">This Month</p>
            <p className="text-base sm:text-xl font-bold text-slate-800 leading-tight">{thisMonthJobs.length}</p>
          </div>
        </div>

        <div className="bg-white rounded-xl border border-slate-200 shadow-card px-2 py-2 sm:px-4 sm:py-3 flex items-center gap-1.5 sm:gap-3">
          <span className="hidden sm:flex h-9 w-9 items-center justify-center rounded-lg bg-blue-50 text-blue-600 flex-shrink-0">
            <CalendarDays className="h-4 w-4" />
          </span>
          <div>
            <p className="text-[10px] sm:text-[11px] text-slate-500 font-medium">Today</p>
            <p className="text-base sm:text-xl font-bold text-slate-800 leading-tight">{todayCount}</p>
          </div>
        </div>

        <div className={`bg-white rounded-xl border shadow-card px-2 py-2 sm:px-4 sm:py-3 flex items-center gap-1.5 sm:gap-3 ${overdueCount > 0 ? "border-red-200" : "border-slate-200"}`}>
          <span className={`hidden sm:flex h-9 w-9 items-center justify-center rounded-lg flex-shrink-0 ${overdueCount > 0 ? "bg-red-50 text-red-600" : "bg-slate-100 text-slate-400"}`}>
            <AlertTriangle className="h-4 w-4" />
          </span>
          <div>
            <p className="text-[10px] sm:text-[11px] text-slate-500 font-medium">Overdue</p>
            <p className={`text-base sm:text-xl font-bold leading-tight ${overdueCount > 0 ? "text-red-600" : "text-slate-800"}`}>{overdueCount}</p>
          </div>
        </div>
      </div>

      {/* ── Two-column: calendar left, detail right ──────────────────────── */}
      <div className="flex flex-col lg:flex-row gap-4 items-start">
        ...
        <div className="flex-1 min-w-0 w-full space-y-4">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-card overflow-hidden">
            ...
            {/* Toolbar */}
            <div className="flex items-center justify-between px-3 sm:px-5 py-2.5 sm:py-3 border-b border-slate-100 gap-2">
              <div className="flex items-center gap-0.5 sm:gap-1">
                ...
                <button
                  onClick={goToday}
                  className="ml-0.5 px-2 sm:px-3 py-1 text-xs font-medium rounded-lg border border-slate-200 hover:bg-slate-50 text-slate-600 transition-colors"
                >
                  Today
                </button>
              </div>

              <h3 className="text-xs sm:text-sm font-semibold text-slate-800 text-center flex-1 truncate px-1">
                {heading}
              </h3>
              ...
              <div className="flex items-center rounded-lg border border-slate-200 overflow-hidden flex-shrink-0">
                <button
                  ...
                  className={`px-2 sm:px-2.5 py-1.5 text-xs font-medium flex items-center gap-0.5 sm:gap-1 transition-colors ${
                ...
                <button
                  ...
                  className={`px-2 sm:px-2.5 py-1.5 text-xs font-medium flex items-center gap-0.5 sm:gap-1 transition-colors border-l border-slate-200 ${
            ...
            {/* Day-name header — single letter on mobile, 3-letter on sm+ */}
            <div className="grid grid-cols-7 border-b border-slate-100 bg-slate-50">
              {DAY_SHORT.map((s, i) => (
                <div key={i} className="py-1.5 sm:py-2 text-center">
                  <span className="sm:hidden text-[11px] font-semibold text-slate-400 uppercase">{s}</span>
                  <span className="hidden sm:block text-[11px] font-semibold text-slate-400 uppercase tracking-wide">{DAY_MEDIUM[i]}</span>
                </div>
              ))}
            </div>
                ...
                {monthCells.map((day, idx) => {
                  if (!day) {
                    return (
                      <div
                        key={`empty-${idx}`}
                        className="min-h-[52px] sm:min-h-[88px] border-b border-r border-slate-100 bg-slate-50/50 last:border-r-0"
                      />
                    );
                  }
                  ...
                  return (
                    <div
                      ...
                      className={`min-h-[52px] sm:min-h-[88px] border-b border-r border-slate-100 last:border-r-0 p-1 sm:p-1.5 text-left hover:bg-slate-50 transition-colors cursor-pointer ${
                        isSelected ? "ring-2 ring-primary ring-inset bg-primary/5" : ""
                      } ${isPast ? "bg-slate-50/60" : ""}`}
                    >
                      {/* Date number */}
                      <span
                        className={`inline-flex h-5 w-5 sm:h-6 sm:w-6 items-center justify-center rounded-full text-[10px] sm:text-xs font-semibold mb-0.5 sm:mb-1 ${
                          isToday
                      ...
                      {dayJobs.length > 0 && (
                        <div className="sm:hidden">
                          <JobDots jobs={dayJobs} today={today} />
                        </div>
                      )}
                      ...
                      {dayJobs.length > 0 && (
                        <div className="hidden sm:block space-y-0.5">
                          {dayJobs.slice(0, 2).map((j) => (
                            <JobChip key={j._id} job={j} today={today} onClick={() => setSelected(cellDate)} />
                          ))}
                          {dayJobs.length > 2 && (
                  ...
                  {weekDays.map((d) => {
                    ...
                    return (
                      <div
                        ...
                        className={`min-h-[100px] sm:min-h-[120px] border-r border-slate-100 last:border-r-0 p-1.5 sm:p-2 cursor-pointer hover:bg-slate-50 transition-colors ${
                          isSelected ? "ring-2 ring-primary ring-inset bg-primary/5" : ""
                        } ${isPast ? "bg-slate-50/60" : ""}`}
                      >
                        {/* Date header */}
                        <div className="flex flex-col items-center mb-1.5 sm:mb-2">
                          ...
                          <span
                            className={`inline-flex h-6 w-6 sm:h-7 sm:w-7 items-center justify-center rounded-full text-xs sm:text-sm font-bold ${
                              isToday
                                ? "bg-primary text-white shadow-sm"
                                : isPast
                                ? "text-slate-350"
                        ...
                        <div className="space-y-0.5">
                          {dayJobs.slice(0, 3).map((j) => (
                            <JobChip key={j._id} job={j} today={today} onClick={() => setSelected(d)} />
                          ))}
                          {dayJobs.length > 3 && (
          ...
          {/* ── Legend ──────────────────────────────────────────────────── */}
          <div className="flex flex-wrap items-center gap-x-4 sm:gap-x-5 gap-y-2 text-xs text-slate-500 pl-1">
        ...
        {/* Mobile: shown below calendar when a date is selected            */}
        {/* Desktop (lg+): slides in as sticky side panel                   */}
        <div
          className={`transition-all duration-300 ease-in-out overflow-hidden w-full lg:flex-shrink-0 ${
            selected ? "lg:w-[340px] opacity-100" : "hidden lg:block lg:w-0 lg:opacity-0"
          }`}
        >
          <div className="w-full lg:w-[340px] lg:sticky lg:top-4">
            <div className="bg-white rounded-xl border border-slate-200 shadow-card overflow-hidden">
              {/* Panel header */}
              ...
              {/* Panel body */}
              <div className="p-3 sm:p-4 space-y-3 max-h-[300px] sm:max-h-[400px] lg:max-h-[calc(100vh-280px)] overflow-y-auto">
