...
async function DisputeSection({ jobId }: { jobId: string }) {
  ...
  const steps = [
    { label: "Submitted",    description: `Raised ${formatRelativeTime(dispute.createdAt)}`, icon: <CheckCircle2 className="h-4 w-4" /> },
    { label: "Under Review", description: "An admin is investigating the issue",              icon: <Search className="h-4 w-4" /> },
    { label: "Resolved",     description: dispute.resolutionNotes ?? "Dispute has been resolved", icon: <ShieldCheck className="h-4 w-4" /> },
  ];
  const activeIdx = dispute.status === "open" ? 0 : dispute.status === "investigating" ? 1 : 2;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="space-y-5">
          ...
          {/* Dispute status */}
          {job.status === "disputed" && <DisputeSection jobId={id} />}
        </div>
      </div>
    </div>
  );
