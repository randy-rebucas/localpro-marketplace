...
export interface InitialClientUser {
  name: string;
  email: string;
  role: string;
  isVerified: boolean;
...
export default function ProfileClient({ initialUser, initialJobCount }: Props) {
  ...
  async function handleAvatarChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const saveData = await saveRes.json() as { avatar?: string; error?: string };
      if (!saveRes.ok) { toast.error(saveData.error ?? "Failed to save avatar"); return; }

      setMe((prev) => ({ ...prev, avatar: saveData.avatar ?? null }));
  ...
  async function saveName(e: React.FormEvent) {
    ...
    try {
      ...
      const data = await res.json() as { name?: string; phone?: string | null; error?: string };
      if (!res.ok) { toast.error(data.error ?? "Failed to update details"); return; }
      setMe((prev) => ({ ...prev, name: data.name ?? prev.name, phone: data.phone ?? null }));
      if (user) setUser({ ...user, name: data.name ?? user.name, phone: data.phone ?? undefined });
  ...
  async function handleAddAddress() {
    ...
    try {
      ...
      if (!res.ok) { toast.error((data as unknown as { error?: string }).error ?? "Failed to add address"); return; }
      setAddresses(data);
      setNewAddressText("");
      setNewAddressCoords(null);
      setNewLabel("Home");
  ...
  async function handleDeleteAddress(id: string) {
    ...
    try {
      ...
      const data = await res.json() as IAddress[] & { error?: string };
      if (!res.ok) { toast.error((data as unknown as { error?: string }).error ?? "Failed to remove address"); return; }
      setAddresses(data);
  ...
  async function handleSetDefault(id: string) {
    ...
    try {
      ...
      const data = await res.json() as IAddress[] & { error?: string };
      if (!res.ok) { toast.error((data as unknown as { error?: string }).error ?? "Failed to update"); return; }
      setAddresses(data);
    } finally {
  ...
  }, [me.avatar, me.kycStatus, me.phone, addresses]);
