...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const adminCandidates = await disputeRepository.findByEscalationLevel("provider", hours48Ago);

    for (const d of adminCandidates) {
      const disputeId = d._id.toString();
      try {
        await disputeRepository.updateById(disputeId, {
        ...
        log.error({ err, disputeId }, "Failed to escalate dispute to admin");
      }
    }

    // ── Escalate admin → PESO (disputes at admin level > 96h since creation) ──
    const pesoCandidates = await disputeRepository.findByEscalationLevel("admin", hours96Ago);

    for (const d of pesoCandidates) {
      const disputeId = d._id.toString();
      try {
        await disputeRepository.updateById(disputeId, {
        ...
        log.error({ err, disputeId }, "Failed to escalate dispute to PESO");
      }
    }

    log.info({ escalatedToAdmin, escalatedToPeso }, "Dispute escalation cron completed");

    return Response.json({ ok: true, escalatedToAdmin, escalatedToPeso });
  } catch (err) {
