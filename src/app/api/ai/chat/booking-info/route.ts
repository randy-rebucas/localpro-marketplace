...
export async function POST(req: NextRequest) {
  try {
    ...
    // Find relevant FAQs using keyword matching
    ...
    if (relevantFAQs.length > 0) {
      // Build response from FAQ database
      let responseMessage = "";
      for (const faq of relevantFAQs) {
        responseMessage += `**${faq.title}**\n\n${faq.answer}\n\n`;
      }

      if (relevantFAQs[0]?.links && relevantFAQs[0].links.length > 0) {
    ...
    // If no FAQ match, use AI to generate helpful response
