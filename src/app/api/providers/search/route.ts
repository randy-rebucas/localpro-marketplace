...
export async function GET(request: NextRequest) {
  const rl = await checkRateLimit(`prov-search:${clientIp(request)}`, { windowMs: 60_000, max: 30 });
  ...
  try {
    ...
    const limit  = Math.max(1, Math.min(PAGE_SIZE, parseInt(searchParams.get("limit") ?? String(PAGE_SIZE), 10) || PAGE_SIZE));
