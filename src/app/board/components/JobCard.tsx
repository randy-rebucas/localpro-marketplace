...
import { MapPin, CalendarDays, Briefcase, Copy, Check, Share2, Zap, ExternalLink } from "lucide-react";
...
export function JobCard({ job }: { job: BoardJob }) {
  ...
  const badges = (
    <div className="flex items-center flex-wrap gap-1">
      <span className="inline-block px-2 py-0.5 rounded-full bg-blue-500/20 text-blue-300 text-[11px] font-semibold uppercase tracking-wider">
        {job.category}
      </span>
      {freshlyPosted && (
        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 text-[11px] font-semibold">
          <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
          New
        </span>
      )}
      {job.isPriority && (
        <span className="inline-flex items-center gap-0.5 px-2 py-0.5 rounded-full bg-red-500/20 text-red-300 border border-red-500/30 text-[11px] font-semibold">
          <Zap className="h-2.5 w-2.5" />
          Priority
        </span>
      )}
      {job.jobSource && job.jobSource !== "private" && (
        <span className="inline-block px-2 py-0.5 rounded-full bg-violet-500/20 text-violet-300 border border-violet-500/30 text-[11px] font-semibold">
  ...
  /** Shared share drawer */
  const shareDrawer = showShare && (
    <div className="border-t border-white/10 px-3 pt-2.5 pb-3 flex items-center justify-between gap-2 bg-black/10">
      <div className="flex items-center gap-1 min-w-0">
        <Share2 className="h-3 w-3 text-slate-500 flex-shrink-0" />
        <span className="text-[10px] text-slate-500 font-medium whitespace-nowrap hidden xs:block">Share:</span>
      </div>
  ...
  return (
    <div className="bg-white/[0.07] border border-white/10 rounded-2xl overflow-hidden hover:bg-white/[0.10] transition-colors flex flex-col">
      ...
      {/* ── Mobile layout (< sm): stacked, no QR, tap-to-apply CTA ── */}
